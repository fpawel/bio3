...
type DeviceInfoProvider struct {
	GoodProduct       func (ps.Product) bool
	BadProduct        func  (ps.Product) bool
	FormatProductType func (int) string
}

func NewPartiesTreeViewModel(db ps.DB, m DeviceInfoProvider) (x *TreeModel) {

	db.View(func(tx ps.Tx) {
		x = &TreeModel{}
		tree := make(map[int]map[time.Month]map[int][]ps.Party)
		for _, p := range tx.Parties() {
			...
			if _, ok := tree[y]; !ok {
				tree[y] = make(map[time.Month]map[int][]ps.Party)
			}

			if _, ok := tree[y][m]; !ok {
				tree[y][m] = make(map[int][]ps.Party)
			}
