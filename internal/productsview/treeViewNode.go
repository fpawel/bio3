...
import (
	"fmt"
	ps "github.com/fpawel/bio3/internal/products"
...
type Node interface {
	walk.TreeItem
	ContainsParty(p ps.PartyTime) bool
	What() string
}
...
func (x *NodeMonth) Text() string {
	return fmt.Sprintf("%s", utils.MonthNumerToName(x.month))
}
...
func (x *NodeMonth) What() string {
	return fmt.Sprintf("Каталог %s %d", utils.MonthNumerToName(x.month), x.parentYearNode)
}
...
func (x *NodeDay) What() string {
	return fmt.Sprintf("Каталог %d %s %d", x.day, utils.MonthNumerToName(x.Month()), x.Year())
}
...
func (x *NodeParty) Party() ps.PartyInfo {
	return x.party
}
...
func (x *NodeProduct) Party() ps.PartyInfo {
	return x.parentPartyNode.Party()
}
func (x *NodeProduct) Product() ps.ProductInfo {
	return x.product
}
...
func (x *NodeYear) ContainsParty(p ps.PartyTime) bool {
	return time.Time(p).Year() == x.year
}
func (x *NodeMonth) ContainsParty(p ps.PartyTime) bool {
	t := time.Time(p)
	return t.Year() == x.Year() && t.Month() == x.Month()
}
func (x *NodeDay) ContainsParty(p ps.PartyTime) bool {
	t := time.Time(p)
	return t.Year() == x.Year() && t.Month() == x.Month() && t.Day() == x.Day()
}
func (x *NodeParty) ContainsParty(p ps.PartyTime) bool {

	return time.Time(p).Equal( time.Time(x.party.PartyTime) )
}
func (x *NodeProduct) ContainsParty(p ps.PartyTime) bool {
	return false
}
...
var ImgCalendarYearPng16, ImgCalendarMonthPng16, ImgCalendarDayPng16,
	ImgErrorPng16, ImgPartyNodePng16, ImgCheckmarkPng16, ImgProductNodePng16,
	ImgWindowIcon walk.Image
