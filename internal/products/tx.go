...
import (
	...
	"github.com/fpawel/bio3/internal/boltdbutils"
...
func (x Tx) BucketRead(path [][]byte) *bolt.Bucket {
	sr := boltdbutils.NewBucketSearcher(x.tx, false)
	sr.Find(path)
	if sr.Error() == boltdbutils.ErrorBucketNotExist {
		return nil
	}
	if sr.Error() != nil {
		panic(sr.Error())
...
func (x Tx) BucketWrite(path [][]byte) *bolt.Bucket {
	sr := boltdbutils.NewBucketSearcher(x.tx, true)
	sr.Find(path)
	if sr.Error() != nil {
		panic(sr.Error())
	}
