...
import (
	...
	u "github.com/fpawel/bio3/internal/utils"
	"sort"
	"time"
...
func collectBucketLogs( buck *bolt.Bucket, logs Logs ) Logs {
	...
	walkLogs( buck, nil, func(_ *bolt.Bucket, k []byte,  r *LogRecord) {
		logs[time.Unix(0, u.BytesToInt64(k))] = r
	})
	return logs
}
...
func bucketLogs2( buck *bolt.Bucket ) (t1 time.Time, r1 *LogRecord, t2 time.Time, r2 *LogRecord) {
	...
	var f bool
	walkLogs( buck, nil,  func(_ *bolt.Bucket, k []byte, r *LogRecord)  {
		t := time.Unix(0, u.BytesToInt64(k))
		if f {
			if t.Before(t1) {
				t1 = t
				r1 = r
...
func (x Tx) WriteTestLog(p DBPath, r WriteTestLogInfo) []byte {
	path := TestsPath(p,r.What).Path()
	if r.Mode == RewriteLog {
		x.ClearLogs(path)
	}
